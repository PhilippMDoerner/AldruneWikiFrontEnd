...
export class ItemResolver extends BaseArticleResolver {
  constructor(service: ItemService, routing: RoutingService) {
...
export class ItemCharacterCreationResolver implements Resolve<ItemObject> {
  ...
  async resolve(
    ...
    const itemOwnerCharacter: CharacterObject = await this.characterService
      .readByParam(currentCampaign.name, { name: ownerName })
      .pipe(first())
      .toPromise();
