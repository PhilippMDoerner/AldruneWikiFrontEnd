...
export class LocationUpdateResolver extends BaseArticleUpdateResolver {
    ...
    async resolve(
        ...
        const modelData: Promise<any> = this.isUpdateRoute(state) ? this.fetchData(campaign, queryParameters) : this.createLocationData(state, campaign, queryParameters);
    ...
    async createLocationData(state: RouterStateSnapshot, campaign: CampaignOverview, queryParameters: any): Promise<LocationObject>{    
        ...
        if(this.isCreateRouteWithParentLocation(state, queryParameters)){
            const parentLocation: LocationObject = await this.fetchParentLocationData(campaign.name, queryParameters);
    ...
    async fetchParentLocationData(campaignName: string, queryParameters: any): Promise<LocationObject>{
        try{
            return await this.service.readByParam(campaignName, queryParameters).toPromise();
        } catch (e){
     ...
     * location or not.
     */
    isCreateRouteWithParentLocation(state: RouterStateSnapshot, queryParameters: {locationName: string, parentLocationName: string}): boolean{
        const currentUrl: string = state.url;
        const isCreateUrl: boolean = currentUrl.endsWith("create");
        const hasParentLocation: boolean = queryParameters.locationName != null && queryParameters.parentLocationName != null;
...
export class LocationMapCreateResolver implements Resolve<LocationObject> {
    constructor(
        private campaignService: GlobalUrlParamsService,
    ){}
    ...
    async resolve(
        ...
        const currentCampaign: CampaignOverview = await this.campaignService.getCurrentCampaign();
