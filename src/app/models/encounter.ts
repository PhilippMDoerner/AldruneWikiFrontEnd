...
export class EncounterObject implements Encounter {
  ...
  getAbsoluteRouterUrl(): string {
    if (
      this.diaryentry_details.session_number == null ||
      this.diaryentry_details.author_name == null ||
      this.diaryentry_details.is_main_session == null
    ) {
  ...
  getAbsoluteLocationRouterUrl(): string {
    if (!this.location_details)
      throw "Can't generate URL for Location of Encounter object without location_details !";
    const parentName =
      this.location_details?.parent_location_name || Constants.NONE_STRING;
