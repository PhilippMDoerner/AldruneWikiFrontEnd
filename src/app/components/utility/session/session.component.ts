...
export class SessionComponent extends CardFormMixin implements OnInit {
  ...
  formlyFields: FormlyFieldConfig[] = [
    this.formlyService.genericCheckbox({
      key: 'is_main_session',
      defaultValue: true,
      label: 'Main Session?',
    }),
    ...
    this.formlyService.genericDatepicker({
      key: 'session_date',
  ...
  constructor(
    ...
    super(warnings, sessionService, route, tokenService);
  }
  ...
  updateDynamicVariables(
    ...
    const diaryentries: { name: string; author_name: string }[] =
      articleData.diaryentries;
    if (diaryentries != null) {
      this.diaryentryUrls = diaryentries.map((diaryentry) =>
