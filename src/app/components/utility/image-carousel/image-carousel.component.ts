...
export class ImageCarouselComponent extends PermissionUtilityFunctionMixin {
  ...
  onSlide(event) {
    ...
    const currentImageIndexStr: string = currentImageSlideId.replace(
      'imageIndex-',
      ''
    );
  ...
  onSubmit(): void {
    if (this.formState === FormState.CREATE) {
      ...
      this.formState === FormState.UPDATE ||
      this.formState === FormState.UPDATEOUTDATED
    ) {
      this.updateImage();
    } else {
  ...
  async updateImage(): Promise<void> {
    ...
    const isUpdateWithImageChange: boolean =
      this.userModel.image.constructor.name.toLowerCase() !== 'string';

    let updateObservable: Observable<ImageObject>;
    if (isUpdateWithImageChange) {
      updateObservable = this.imageUploadService.update(
        this.userModel.pk,
        this.userModel
      );
    } else {
      const minimizedUserModel = {
        pk: this.userModel.pk,
        name: this.userModel.name,
      } as ImageObject;
      updateObservable = this.imageUploadService.patch(
        this.userModel.pk,
        minimizedUserModel
      );
    }
